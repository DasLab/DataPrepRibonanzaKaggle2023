...
%% Data v2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
data_v2_version = 'v2.3.2';  % Fix Usage labels in test set, prepare copies of all 4 important files.
%outdir = sprintf('data_sets/kaggle2023_v2/%s',data_v2_version);
outdir = sprintf('~/Desktop/%s',data_v2_version); % to avoid Google Drive issues.
...
infiles = dir( 'data_sets/kaggle2023_v2/train/train*.csv' );
t_train = concatenate_tables(infiles);
t_train = renamevars(t_train,'id','sequence_id')
check_dataset_stats(t_train);

outfile = sprintf('%s/train_data.%s.csv',outdir,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_train),outfile);
writetable(t_train,outfile);
strip_nan(outfile);

%% save for sanity checks:
% train_sequence = t_train.sequence;
% train_sequence_id = t_train.sequence_id;
% save train_sequence.mat train_sequence train_sequence_id
...
outfile = sprintf('%s/data_test_KEEP_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version);
...
outfile = sprintf('%s/solution_test_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_solution_test),outfile);
tic
writetable(t_solution_test,outfile);
toc

%% Prepare sample_submission -- not needed now, prepare from concatenated solution file below.
% t_sample = removevars(t_solution_test,{'Usage'}); 
% t_sample.reactivity_DMS_MaP = repmat(0.0,height(t_sample),1);
% t_sample.reactivity_2A3_MaP = repmat(0.0,height(t_sample),1);
% outfile = sprintf('%s/sample_submission_test.%s.csv',outdir,data_v2_version);
% fprintf('Outputting %d rows to %s.\n',height(t_sample),outfile);
% tic
% writetable(t_sample,outfile);
% toc
% gzip(outfile)
...
%%
outfile = sprintf('%s/test_sequences_not_future.%s.csv',outdir,data_v2_version);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Get the future_1Mround2 data 
%% This copies code from above, but
%% seeing crashes when running in function
%% due to MATLAB memory management problems
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
infiles = dir( 'data_sets/kaggle2023_v2/test_private_future/test_future_1M*2A3*.csv' );
t_test_future_1Mround2_2A3 = concatenate_tables(infiles);
t_test_future_1Mround2_2A3 = create_Usage_column( t_test_future_1Mround2_2A3, 'Private' );

infiles = dir( 'data_sets/kaggle2023_v2/test_private_future/test_future_1M*DMS*.csv' );
t_test_future_1Mround2_DMS = concatenate_tables(infiles);
t_test_future_1Mround2_DMS = create_Usage_column( t_test_future_1Mround2_DMS, 'Private' );

%% checks...
assert( length(unique(t_test_future_1Mround2_2A3.sequence)) == height(t_test_future_1Mround2_2A3));
assert( length(unique(t_test_future_1Mround2_DMS.sequence)) == height(t_test_future_1Mround2_DMS));

%% Output test_sequences.csv and sample_submissions.csv
tic
tables = { t_test_future_1Mround2_2A3, t_test_future_1Mround2_DMS  };
t_test_all = concatenate_tables( tables );
t_test_all = renamevars(t_test_all,'id','sequence_id');
check_dataset_stats(t_test_all);
toc
...
%% Save files with all the information
outfile = sprintf('%s/data_future_1Mround2_KEEP_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_test_all),outfile);
writetable(t_test_all,outfile);
strip_nan(outfile);
...
%% Output this solution file!
outfile = sprintf('%s/solution_future_1Mround2_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_solution_test),outfile);
tic
writetable(t_solution_test,outfile);
toc
...
t_test_sequence = table(id_min,id_max,sequence_id,sequence,future);
%%
outfile = sprintf('%s/test_sequences_future_1Mrounds.%s.csv',outdir,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_test_sequence),outfile);
tic; writetable(t_test_sequence,outfile); toc
tic; gzip(outfile); toc
...
%% due to MATLAB memory management problems
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
infiles = dir( 'data_sets/kaggle2023_v2/test_private_future/test_future_Positives*2A3*.csv' );
...
infiles = dir( 'data_sets/kaggle2023_v2/test_private_future/test_future_Positives*DMS*.csv' );
...
%% Save files with all the information
outfile = sprintf('%s/data_future_Positives_KEEP_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_test_all),outfile);
writetable(t_test_all,outfile);
strip_nan(outfile);
...
%% Output this solution file!
outfile = sprintf('%s/solution_future_Positives_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_solution_test),outfile);
tic
writetable(t_solution_test,outfile);
toc
...
t_test_sequence = table(id_min,id_max,sequence_id,sequence,future);
%%
outfile = sprintf('%s/test_sequences_future_Positives.%s.csv',outdir,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_test_sequence),outfile);
tic; writetable(t_test_sequence,outfile); toc
%tic; gzip(outfile); toc
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
csvfiles = { sprintf('%s/test_sequences_not_future.%s.csv',outdir,data_v2_version),...
    sprintf('%s/test_sequences_future_1Mround2.%s.csv',outdir,data_v2_version),...
    sprintf('%s/test_sequences_future_Positives.%s.csv',outdir,data_v2_version) };
...
outfile = sprintf('%s/test_sequences.%s.csv',outdir,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_test_sequence),outfile);
tic; writetable(t_test_sequence,outfile); toc
tic; gzip(outfile); toc
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
csvfiles = { sprintf('%s/solution_test_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version),...
    sprintf('%s/solution_future_1Mround2_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version),...
    sprintf('%s/solution_future_Positives_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version) };
...
outfile = sprintf('%s/solution_CONFIDENTIAL.%s.csv',outdir_CONFIDENTIAL,data_v2_version);
...
t_sample.reactivity_DMS_MaP = repmat(0.0,height(t_sample),1);
t_sample.reactivity_2A3_MaP = repmat(0.0,height(t_sample),1);
outfile = sprintf('%s/sample_submission.%s.csv',outdir,data_v2_version);
fprintf('Outputting %d rows to %s.\n',height(t_sample),outfile);
